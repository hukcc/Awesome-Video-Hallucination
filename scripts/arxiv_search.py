...
def generate_new_papers_md(papers, output_path):
    ...
    for paper in papers:
        ...
        section.append(f'### [{paper["title"]}]({paper["url"]})')
        section.append("")
        section.append(f'- **arXiv ID:** `{paper["arxiv_id"]}`')
        section.append(f'- **Date:** {paper["date"]}')

        if paper["github_links"]:
            links_str = " | ".join(
                f"[Code]({link})" for link in paper["github_links"]
            )
            section.append(f"- **Code:** {links_str}")
        else:
            section.append("- **Code:** -")

        section.append(f"- **Abstract:** {abstract_highlighted}")
        section.append("")
        section.append("---")
        section.append("")
    ...
    if existing and not existing.isspace():
        body = existing
        if body.startswith("# "):
